...
## Incident-01 — KRS War notification loop fix — 2026-08-03

- [x] Diagnose 25-minute WhatsApp spam burst (±2 pesan/~12 detik, ±226 kiriman) to admin.
- [x] Eliminate reminder scheduler, os_jobs/prayer, and echo-loop hypotheses with evidence.
- [x] Identify root cause: `run_krs_war_if_armed` called `notify_admin` for
  `krs_war_started`/`krs_war_taken`/`krs_war_error` on every watcher tick
  (10s window) without a state guard while KRS war stayed `partial`.
- [x] Remove stale reminder rows 18/19 (backup: /tmp/opencode/xninetzy.sqlite3.bak-20260803-073536).
- [x] Guard notifications by first-run-per-window and status transitions in krs_war.py.
- [x] Verify: 21 KRS war tests passed, ruff passed, container restart applied the fix,
  `send_text_message` new count = 0 over 2+ minutes after restart.

Full analysis: `docs/plan/20260803_krs_war_notification_loop_fix.md`.

## Audit-01 — Lightning improvement audit — 2026-08-03

- [x] Snapshot Lightning tables (415 episodes, 408 actions).
- [x] Document 6 gaps: groundedness=0.0, suspicious 0% failure, evidence_status
  unused in reward, 11 hanging active episodes, cold-start strategy_rank bias,
  risk class always read.
- [x] Implement reward-v2 coverage/confidence/missing-component reporting so a
  successful tool return without evidence cannot produce a perfect reward.
- [x] Derive MCP risk class from the canonical tool manifest instead of hard-coding read.
- [ ] Complete G4 sweep+idempotency, G2 error-path observability, and G5/G1
  shadow policy/groundedness rollout per `docs/plan/lightning_improvement_plan.md`.

## Batch Academic-UACC-01 — UACC / UnairSatu SSO portal access — 2026-08-05

- [x] Add `UACC_ENABLED`/`UACC_BASE_URL`/`UACC_CREDENTIAL_SOURCE` settings and
  `.env.example` entries; UACC shares the HEBAT credential source.
- [x] Add `SiteDefinition "uacc"` with `allowed_hosts` for the SSO host
  (`unairsatu.unair.ac.id`); `is_allowed_url` now enforces site hostnames.
- [x] Register `uacc` as a web-analysis portal slug and add `--site uacc` CLI
  choice; SSO root redirect chain verified (`/mhs` -> cybercampus ->
  unairsatu -> `/site/login`).
- [x] Extend `login_coordinator` with `site_slug`-aware config and challenge
  flow; optional CAPTCHA behavior approved by owner: no CAPTCHA -> direct
  login, CAPTCHA -> manual WhatsApp challenge (never auto-solved).
- [x] Add six admin-only UACC tools (`uacc_login_start`,
  `uacc_login_submit_captcha`, `uacc_login_cancel`, `uacc_session_status`,
  `uacc_logout`, `uacc_info`) registered in the canonical registry.
- [x] Route `/uacc-login`, `/uacc-captcha <id> <jawaban>`,
  `/uacc-login-cancel <id>` in the WhatsApp command router with site-aware
  CAPTCHA captions; `/web-analysis|refresh|pages uacc` supported.
- [x] Live verification: UACC login succeeded via manual CAPTCHA (challenge
  answered by owner), `web_analysis_refresh("uacc", authenticated=True)`
  analyzed login module + page structure for `/` and `/mhs` (public and
  authenticated) and stopped safely at `human_verification_required`.
- [x] Persist analysis to Graph RAG node `[web_site] "UACC SSO (UnairSatu
  UNAIR)"` and create skill `xninetzy-uacc` v1.0.0.
- [x] Verification: 44 focused UACC/login/web-analysis tests passed, full AI
  suite 701 passed, Ruff clean.

Open: AI container image predates the `/uacc-*` router patterns
(`command_router.py` inside the image is 05:43 UTC while host file is 07:59
UTC); rebuild + restart `xninetzy-ai-1` is required before `/uacc-captcha`
works over WhatsApp. MCP host server already runs the new code, so MCP tools
are active.
