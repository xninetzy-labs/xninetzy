...
CLI memakai SSE `/api/chat/stream` dengan request ID per pesan. Hanya satu request, `AbortController`, listener stream, dan timer Thinking yang aktif untuk satu giliran. Delta jawaban dibuffer sebelum dirender dan respons aktif memakai preview visual terbatas. Setelah selesai, jawaban lengkap dipindahkan ke transcript statis Ink sehingga riwayat tidak ikut digambar ulang oleh spinner atau timer. Event lama diabaikan bila request sudah selesai atau dibatalkan.
...
Ink memakai reconciliation React untuk memperbarui terminal. Flicker sebelumnya berasal dari timer status dan backdrop animasi yang mengubah parent tree, perubahan properti composer selama request, serta pembaruan delta tanpa lifecycle request yang ketat. Implementasi saat ini mempertahankan ID widget dan callback input, memakai transcript statis untuk header serta pesan selesai, dan membatasi tick 100 ms dan 200 ms ke glyph spinner serta label waktu. Satu listener resize dimiliki root, streaming dibuffer 50 ms, preview aktif dibatasi berdasarkan lebar terminal, dan transcript lengkap baru dicetak satu kali setelah completion. Activity detail menampilkan empat event terbaru agar area dinamis tidak pernah memaksa Ink membersihkan seluruh terminal.
