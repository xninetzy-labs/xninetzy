...
async def _h_sync(action, state):
    try:
        from app.tools.hebat.tools import hebat_sync_courses
...
async def _h_course_detail(action, state):
    ...
    try:
        from app.tools.hebat.tools import hebat_sync_course_activities
...
async def _h_assignment_detail(action, state):
    ...
    try:
        from app.tools.hebat.tools import hebat_get_assignment_detail
...
async def _h_download(action, state):
    ...
    try:
        from app.tools.hebat.tools import hebat_download_material
...
def _make_search_handler(kind: str) -> Handler:
    async def handler(action: WorkflowAction, state: WorkflowState) -> WorkflowActionResult:
        ...
        try:
            items: list = []
            if kind == "youtube":
                from app.research.youtube_search import search_youtube as fn  # type: ignore
                items = await fn(topic)  # type: ignore
            elif kind == "web":
                from app.research.web_search import search_web as fn  # type: ignore
                items = await fn(topic)  # type: ignore
...
async def _h_reminder_infer(action, state):
    try:
        from app.reminders.reminder_service import ReminderService, format_auto_reminder_summary
