...
**Checklist:**
- [~] Audit semua tool output untuk JID, cookie, token, path absolut (JID ter-audit 2026-08-24: tidak ada leak f-string langsung; audit cookie/token/path belum)
- [x] Implementasikan masking layer di `core/security.py` (dibuat 2026-08-24: `redact_jids_in_text` + `sanitize_tool_output`, terpasang di choke point `mcp_tool_adapter`)
- [x] Replace `sender_id`, `chat_id`, `Target` dengan label aman (JID otomatis termasking di semua output MCP; identitas trusted context tetap server-side injected)
- [x] Test: tidak ada JID/credential muncul di output tool manapun (9 unit test sanitizer + 2 test integrasi adapter; sweep integrasi per-tool menyusul)
...
**Checklist:**
- [~] Audit semua tool mutasi (lihat tabel di review — ~100 tools) (store generik selesai 2026-08-24; rollout ke sisa mutasi menyusul per-domain)
- [~] Tambah `idempotency_key: str = ""` sebagai optional field (terpasang di `task_capture`, `knowledge_ingest_text` + wrapper MCP sebagai referensi pola)
- [x] Implementasi idempotency store (hash → existing_result) (`db/idempotency.py`: reserve-then-complete, replay hasil, reservation dilepas saat execute gagal; DDL shared dengan migrasi)
- [~] Update dokumentasi untuk tiap tool yang sekarang idempotent (docstring tool sudah; docs/MCP_TOOLS.md menyusul di Milestone 3)
- [x] Test: call yang sama 2x → result sama, data tidak duplikat (7 test hermetik di tests/db/test_idempotency_store.py)
...
**Checklist:**
- [x] Definisikan error contract: `{"error": true, "code": "TOOL_NOT_FOUND", "message": "Task ID 99999 tidak ditemukan", "valid_values": null}` (diadaptasi 2026-08-24 ke format string tunggal `❌ [CODE] message [+ Nilai valid: ...]` agar tetap readable di WhatsApp dan parseable oleh agent via `parse_tool_error`)
- [x] Implementasi error formatter di `tools/` base class (`tools/errors.py`: `ToolErrorCode`, `tool_error()`, `parse_tool_error()`, `ERROR_CODE_PATTERN`; pendekatan modul, bukan base class, sesuai arsitektur tool LangChain yang ada)
- [~] Pastikan semua tool return error dalam format yang sama (vertical slice domain HEBAT selesai: prepare/upload/cancel/remove/download/detail; rollout domain lain menyusul)
- [x] Bedakan: invalid_input vs not_found vs not_configured vs server_error (5 kode: INVALID_INPUT, NOT_FOUND, NOT_CONFIGURED, POLICY_HELD, SERVER_ERROR)
- [~] Test: setiap tool error case return format yang benar (8 test kontrak + slice HEBAT; per-tool penuh mengikuti rollout)
...
**Checklist:**
- [x] Implementasi paper search via arXiv API (free, no key) (modul `os/research/academic_search.py` di-refactor 2026-08-24: fetcher terpisah monkeypatchable + parser murni `parse_arxiv_response`)
- [x] Implementasi paper search via CrossRef API (free, no key) (parser murni `parse_crossref_items` + `_parse_crossref_work` untuk detail)
- [x] Tool: `research_search_papers(query, sources, max_results)` (terdaftar di registry → terekspos otomatis via MCP; total registry kini 257 tool)
- [x] Tool: `research_get_paper(identifier, source)` — DOI, ID/URL arXiv, auto-deteksi jenis identifier
- [~] Tool: `research_download_paper(identifier, save_path)` — **fallback Sci-Hub DIBATALKAN sengaja (hak cipta)**; hanya OA legal: PDF arXiv + link `application/pdf` dari CrossRef; tanpa OA → status jujur `no_oa_pdf`
- [~] Integrasikan hasil ke knowledge base: search paper → ingest langsung (`research_get_paper(..., ingest=True)` menyimpan judul+abstrak; PDF penuh bisa lanjut via `knowledge_ingest_file`)
- [~] Update research brief generator untuk include paper sources (deep_research sudah memakai action `academic_search` internal sejak sebelumnya; verifikasi brief menyusul)
...
**Checklist:**
- [x] Reset: `research_light(query)` → web search + youtube + paper → ringkas (2026-08-24: modul baru `os/research/light_pipeline.py` — collect_quick_sources lintas 3 sumber dgn timestamp `collected_at`, render per-grup 🌐/🎓/📺; pesan provider usang TAVILY/SERPER dihapus)
- [x] Reset: `research_generate_brief(topic)` → subplan → search → brief dengan sumber nyata (kini benar-benar mengumpulkan sumber via action registry untuk topik + 2 subplan pertama; sebelumnya selalu `[]`)
- [~] Reset: `deep_research_topic(topic)` — admin only, dengan session, sumber terverifikasi (struktur sudah ada sejak sebelumnya: gating admin, session DB, action academic via include_academic; verifikasi live menyusul)
- [x] Output: setiap hasil research menyertakan sumber (title, URL, timestamp) (`format_sources_block` kini menampilkan `· diambil <timestamp>` bila ada)
- [x] Test: pipeline lengkap dari topic → search → brief → save (7 test offline di tests/os/research/test_light_pipeline.py: merge 3 sumber, toleransi provider gagal, grouping, render tool, honest-empty, brief+sumber nyata+timestamp, stamp sitasi)
