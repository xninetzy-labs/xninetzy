"""Short owner progress updates between workflow steps.

Keeps the owner informed ("✅ Step 2/6 selesai: …") without spamming: respects
config toggles, a min-interval rate limit, a max message length, and only fires
for "big" actions. Delivery is now via the owner inbox (replaces WA delivery).
Never sends raw tracebacks; detail stays in server logs.
"""
...
class WorkflowNotifier:
    def __init__(self, chat_id: str, *, enabled: bool | None = None, send=None):
        ...
        self._send = send  # injectable for tests; defaults to inbox sender
    ...
    async def _dispatch(self, text: str) -> None:
        ...
        try:
            ...
            logger.info(
                "workflow_notify_sent chat_id=%s len=%d", self.chat_id, len(text)
            )
        except Exception as exc:  # pragma: no cover - delivery failure
            logger.warning(
                "workflow_notify_failed chat_id=%s err=%s", self.chat_id, exc
            )
    ...
    def _sender(self):
        ...
        async def _capture(chat_id: str, text: str) -> None:
            capture_item(text, kind="note", chat_id=chat_id)
