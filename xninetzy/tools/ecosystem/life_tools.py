...
def task_list(status: str | None = None) -> str:
    ...
    if not tasks:
        return "Tidak ada task." + (
            " Coba `/tasks` atau tambah task baru." if not status else ""
        )
...
def task_complete(task_id: int, chat_id: str = "system") -> str:
    ...
    if not complete_task(task_id):
        return f"Task *{t['title']}* sudah selesai."
    record_event(
        chat_id,
        "task_completed",
        "whatsapp",
...
def money_summary(period: str = "month") -> str:
    ...
    cat_lines = (
        "\n".join(f"  • {c['category']}: Rp {c['total']:,.0f}" for c in cats)
        or "  (belum ada)"
    )
    acc_lines = (
        "\n".join(f"  • {a['name']}: Rp {a['balance']:,.0f}" for a in accounts)
        or "  (belum ada)"
    )
...
def workout_summary(period: str = "week") -> str:
    ...
    for sess in sessions[:5]:
        dur = (
            f" ({sess.get('duration_minutes', '?')} min)"
            if sess.get("duration_minutes")
            else ""
        )
...
@tool
def habit_log(
    name: str, value: int = 1, notes: str = "", chat_id: str = "system"
) -> str:
...
@tool
def daily_checkin(
    mood: int, energy: int, focus: int, summary: str, chat_id: str = "system"
) -> str:
...
def daily_review_generate(chat_id: str = "system") -> str:
    ...
    goal_status = (
        "\n".join(f"• {g['title']} ({g.get('status', '?')})" for g in goals[:3])
